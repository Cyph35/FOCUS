-- Canonical schema for FOCUS submissions.
-- Keep this file in sync with supabase/migrations/20260825_create_submissions.sql
-- Source of truth: text response_id (RSP-XXXXXXXX), service-role-only access, no anon policies.
...
create table if not exists public.submissions (
  ...
  constraint submissions_f1_range check (f1 is null or f1 between 1 and 5),
  constraint submissions_f2_range check (f2 is null or f2 between 1 and 5),
  constraint submissions_u1_range check (u1 is null or u1 between 1 and 5),
  constraint submissions_u2_range check (u2 is null or u2 between 1 and 5),
  ...
  constraint submissions_grade_level_values check (grade_level in ('Grade 7', 'Grade 8', 'Grade 9', 'Grade 10', 'Grade 11', 'Grade 12', 'College', '11 Academic-Engineering', '11 Academic-Medical'))
);
...
-- System errors surfaced on the admin Reports page. Same access model as submissions.
